...
- コメント形式: `(コメント)\n本文...` (slope-karteと同じ)
- **画像の格納**: 現テンプレートは **パス文字列**（`"Z:\大竹\Kku27\R0022542.JPG"`）でセル値に入っており、Excel内埋め込み画像は 0 件
  - → APP では画像を追加した枠のセル値を消し、同じ座標に Excel 画像オブジェクトとして埋め込む
- **通番**: BL列 (col 64) に 1〜(撮影枚数) の通し番号
- 実データ件数: パス文字列セル 48 個を検出（= 約16 ページ分既データ）
